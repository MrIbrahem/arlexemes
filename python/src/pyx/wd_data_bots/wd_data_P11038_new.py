...
def get_lemmas_with(limit=0, offset=0, order="DESC", order_by="id"):
    # Fast
    # ---
    query = """
        SELECT
...
def get_lemmas_without(limit=0, offset=0, order="DESC", order_by="id"):
    # Fast
    # ---
    query_lemmas_without_slow = """
        SELECT
            l.id, l.lemma_id, l.lemma, l.pos, l.pos_cat, l.sama_lemma_id, l.sama_lemma
        FROM
            lemmas_p11038 AS l
        ...
        WHERE
            wdp.value IS NULL AND wdp2.value IS NULL
        LIMIT 200
        OFFSET 50000
    """
    # ---
    query = """
        ...
        WHERE
            l.id NOT IN (
                SELECT l2.id
                FROM lemmas_p11038 AS l2
                    JOIN wd_data_p11038 AS wdp2
                    ON l2.lemma_id = wdp2.value
                    #JOIN wd_data AS w ON wdp2.wd_data_id = w.wd_id
            )
        AND
            l.id NOT IN (
                SELECT l2.id
                FROM lemmas_p11038 AS l2
                    JOIN wd_data_p11038 AS wdp2
                    ON l2.sama_lemma_id = wdp2.value
                    #JOIN wd_data AS w ON wdp2.wd_data_id = w.wd_id
            )
    """
    # ---
    query = """
        ...
        FROM
            lemmas_p11038 AS l
        WHERE
            l.lemma_id NOT IN ( SELECT value FROM wd_data_p11038 )
        AND
            l.sama_lemma_id NOT IN ( SELECT value FROM wd_data_p11038 )
    """
    # ---
    params = []
    # ---
...
def get_lemmas(limit=0, offset=0, order="DESC", order_by="id", filter_data="with"):
    ...
    if filter_data == "without":
        logs, db_exec_time = get_lemmas_without(limit, offset, order, order_by)
    else:
        logs, db_exec_time = get_lemmas_with(limit, offset, order, order_by)
    # ---
    return logs, db_exec_time
