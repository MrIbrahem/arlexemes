...
REPO_URL="https://github.com/MrIbrahem/arlexemes.git"
...
REQUIREMENTS_PATH="$CLONE_DIR/requirements.txt"
if [ ! -f "$REQUIREMENTS_PATH" ]; then
    REQUIREMENTS_PATH="$CLONE_DIR/src/requirements.txt"
fi

# Activate the virtual environment and install dependencies from the clone
if [ -f "$REQUIREMENTS_PATH" ]; then
    cp "$REQUIREMENTS_PATH" "$TARGET_DIR/requirements.txt"
